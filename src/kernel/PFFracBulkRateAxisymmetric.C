...
PFFracBulkRateAxisymmetric::PFFracBulkRateAxisymmetric(const InputParameters & parameters):
  Kernel(parameters),
...
Real
PFFracBulkRateAxisymmetric::computeQpResidual()
{
    Real gc = _gc_prop[_qp];
    Real c = _u[_qp];
    Real x = _test[_i][_qp] * ( 2.0*(1.0-c)*(1 - _kdamage) * _G0_pos[_qp]/gc - c/_l );

    return - x/_visco;   
}

Real
PFFracBulkRateAxisymmetric::computeQpJacobian()
{
    Real gc = _gc_prop[_qp];
    Real c = _u[_qp];

    return ( _test[_i][_qp] * (2.0*(1 - _kdamage)* _G0_pos[_qp]/gc + 1.0/_l) * _phi[_j][_qp] )/_visco;


}

Real
PFFracBulkRateAxisymmetric::computeQpOffDiagJacobian(unsigned int jvar)
{
  unsigned int c_comp;
  ...
  Real x = 2.0*(1.0-c) * (1 - _kdamage) * (_G0_pos[_qp]/gc) - c/_l;

  Real xfac = - 1.0 / _visco * 2.0 * (1.0 - c) * (1 - _kdamage) / gc;
